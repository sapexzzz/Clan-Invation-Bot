...
### Added
- `bot/middleware.py` — `BlockMiddleware`: intercepts **all** updates (messages and button presses) from banned users before they reach any handler
- A banned user receives `🚫 You are banned. Access to the bot is restricted.` for any interaction

### Changed
- Removed the duplicate ban check from `start.py` — now centralized in middleware
- Updated `README_RU.md` and `README_EN.md`: added Middleware and ClanPanel sections, updated project structure and stack

### Fixed
- Banning via the group button previously did not prevent the user from sending direct messages to the bot
